"""
Supervisor evaluator for the code_change_agent (executor side of the pair).

Uses build_pair_evaluation_context() to select up to 10 tasks where
code_change_agent participated, then asks the LLM to assess the agent's
behavioral patterns (scope discipline, declaration accuracy, retry improvement).

The validator evaluator (code_change_agent_validator_evaluator.py) shares the same
task selection and produces a separate AgentEvaluationOutput.
"""
...
CODE_CHANGE_AGENT_EVALUATOR_SYSTEM_PROMPT = prompt_loader.get("code_change_agent_evaluator")
...
def _build_user_prompt(
    *,
    ...
    return f"""
Evaluate the code_change_agent's behavioral patterns for project {project_id}.
...
Each task contains:
- task_status: final pipeline outcome (ground truth)
- runs[]: each attempt — work_summary, completed_scope, remaining_scope, changed_files (paths),
  blockers_found, joint_validation_decision, validator_result.decision/summary (cross-context)

Identify behavioral patterns across all tasks.
...
- findings: prose description of behavioral patterns (minimum 20 characters)
- issues: list of specific concrete problems (empty list if none)
- suggestions: list of actionable improvement suggestions (empty list if none)
""".strip()


class CodeChangeAgentEvaluator:
    """Evaluates the code_change_agent's behavioral patterns for a project."""
    ...
    def evaluate(
        ...
        *,
        ...
        raw = provider.generate_structured(
            system_prompt=CODE_CHANGE_AGENT_EVALUATOR_SYSTEM_PROMPT,
            user_prompt=user_prompt,
            schema_name="code_change_agent_evaluator_output",
