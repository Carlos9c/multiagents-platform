"""
Supervisor evaluator for the security_qa_agent.

Reads QA session records where security_qa_agent ran and assesses how well the
agent applied the OWASP Top 10 (2021) checklist: check coverage completeness,
finding accuracy, and whether findings cite specific code evidence.
"""
...
_SYSTEM_PROMPT = prompt_loader.get("security_qa_agent_evaluator")

_AGENT_NAME = "security_qa_agent"


def _build_user_prompt(*, project_id: int, ctx: dict) -> str:
    prompt_loader.validate_builder_inputs(
        "security_qa_agent_evaluator",
        "main",
        {
    ...
    return (
        ...
        "findings (with category='security'), findings_summary.\n\n"
        f"Assess OWASP checklist coverage completeness and finding evidence quality "
        f"for the {_AGENT_NAME}."
    )


class SecurityQAAgentEvaluator:
    """Evaluates the security_qa_agent's OWASP coverage and finding precision."""
    ...
    def evaluate(
        ...
        *,
        ...
        raw = provider.generate_structured(
            ...
            schema_name="security_qa_agent_evaluator_output",
        ...
        try:
            ...
            logger.warning("security_qa_agent_evaluator_validation_failed exc=%s", exc)
