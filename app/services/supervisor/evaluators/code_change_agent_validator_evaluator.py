"""
Supervisor evaluator for the code_change_agent_validator (validator side of the pair).

Uses the same task selection as CodeChangeAgentEvaluator but builds a validator-focused
prompt that assesses calibration accuracy, finding specificity, and partial_annotation
actionability.
"""
...
CODE_CHANGE_AGENT_VALIDATOR_EVALUATOR_SYSTEM_PROMPT = prompt_loader.get(
...
def _build_user_prompt(
    *,
    ...
    return f"""
Evaluate the code_change_agent_validator's calibration and findings quality for project {project_id}.
...
Each run's validator_result contains the validator's decision, findings, and partial_annotations.
Executor cross-context (work_summary, changed_files paths) is included for reference.

Assess calibration, finding specificity, partial_annotation actionability, scope discipline,
and consistency across retry attempts.
    ...
    def evaluate(
        ...
        *,
        ...
        raw = provider.generate_structured(
            system_prompt=CODE_CHANGE_AGENT_VALIDATOR_EVALUATOR_SYSTEM_PROMPT,
            user_prompt=user_prompt,
            schema_name="code_change_agent_validator_evaluator_output",
