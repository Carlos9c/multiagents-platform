"""
Supervisor evaluator for the command_runner_agent_validator (validator side of the pair).

Uses the same task selection as CommandRunnerAgentEvaluator.  The validator-focused
prompt receives full validator_result data (findings, partial_annotations) plus
the executor's command_trace_entries as cross-context.
"""
...
COMMAND_RUNNER_AGENT_VALIDATOR_EVALUATOR_SYSTEM_PROMPT = prompt_loader.get(
...
def _build_user_prompt(
    *,
    ...
    return f"""
Evaluate the command_runner_agent_validator's calibration and findings quality for project {project_id}.
...
Each run contains:
- validator_result: the validator's decision, findings, partial_annotations, blockers
- command_trace_entries[]: the actual commands executed (cross-context for grounding findings)

Assess calibration (decision vs task_status), finding specificity, partial_annotation
actionability, and whether findings are grounded in actual command results.
    ...
    def evaluate(
        ...
        *,
        ...
        raw = provider.generate_structured(
            system_prompt=COMMAND_RUNNER_AGENT_VALIDATOR_EVALUATOR_SYSTEM_PROMPT,
            user_prompt=user_prompt,
            schema_name="command_runner_agent_validator_evaluator_output",
